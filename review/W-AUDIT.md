...
| Exact tested release produces the described representation | Yes | vcfcv **2.1.2** (`3f06d18`) and VCF-RDFizer **3.0.3** (`be658a2`), both tagged and both published as images; every figure regenerated by one run of `reproduce/run.sh` |
...
3. ~~**Container image still lags the source.**~~ — done, `v3.0.3` is published as
   `sha256:31f1361b`. All three release gates are closed.
