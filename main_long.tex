...
A specification-derived assessment of 94 requirements over VCF 4.1--4.5 is replayed over graphs from an independent converter, on which the two producers agree for 508 of 572 checks with no divergence attributable to a representation gap; a semantic round-trip recovers the fixed columns and genotypes of all 35 records from structured properties alone; and one integration example answers an allele-dependent question under VCF 4.5 local alleles with checked answers and recovered provenance. 
The cross-producer replay also found a silent correctness defect in local-allele handling that per-producer testing had not, which we report as evidence for keeping representational mechanism, implementation and correctness apart. 
...
VCF2RDF is the instructive case: its published term set does define \term{Number}, \term{Type} and \term{Description}, so the cardinality rule is in the graph, but with no allele-index term a consumer must still split the list and count ALT alleles by re-reading a literal. 
...
GVO's column reads S throughout because the retrieved ontology declares 48 classes, 12 datatype properties and \emph{no} object properties: a model of variation types, doing something other than file modelling. 
And ``requires additional conventions'' states what an inspected artifact establishes, never that a model's community could not represent something.
...
The assessment above builds its witness graphs with the vocabulary repository's own materializer, whereas the implementation this paper cites is a separate program. 
Both target VCF Core, and nothing had established that a query and expected answer written against one also holds over the other --- which is precisely the gap between ``the vocabulary can express this'' and ``the software people will use does''.

We therefore replayed the existing queries, against the existing reviewed expected answers, over graphs produced by VCF-RDFizer. 
Ten fixtures spanning VCF 4.1--4.5 and the difficult decisions above were each converted in both profiles and every applicable case replayed, giving 572 checks over 143 cases and 39 requirements. 
The two producers agree on 536 checks and both pass on 508. 
Of the 39 requirements, 34 agree on every check and 28 pass outright for both. 
The 28 both-fail checks are the condensed profile's documented storage trade-off, on which the producers agree before and after.

The exercise earned its keep by failing. 
Its first run diverged on 15 requirements, nine of which were converter defects or gaps precise enough to fix. 
One was a silent correctness defect in VCF 4.5 local alleles, this paper's own headline feature: \term{Number=LA} and \term{Number=LR} items were bound to allele indices positionally, ignoring the sample's \texttt{LAA} subset, so a depth list of \texttt{20,30,10} under \texttt{LAA=2,4} was attached to alleles 0, 1 and 2 instead of 0, 2 and 4. 
The values were plausible and well-formed; they were on the wrong alleles. 
No existing test caught it because every existing test ran against the other producer. 
Five requirements still diverge: a QUAL datatype the vocabulary deliberately leaves open, two structured accessors the converter has not built, one condensed-vector decoding path and one empty-list encoding convention. 
...
For each of the ten fixtures we reconstruct CHROM, POS, ID, REF, ALT, QUAL, FILTER and the GT subfield from the RDF alone and compare with the source file: REF and ALT are rebuilt from ordered \term{alleleIndex}/\term{alleleValue} resources and GT from ordered \term{GenotypeAlleleCall} resources with their phasing status, so no untouched source string serves as a shortcut. 
All 35 records are recovered. 
This is a partial recovery by design: INFO and the non-GT FORMAT subfields are out of scope, and it is not a claim of whole-file reconstruction.
...
Across the 39 exercised requirements the profiles return the same outcome on 252 of 286 comparable checks, and the 34 disagreements are not scattered: every one is a genotype- or sample-level question. 
File, header, record and allele questions agree on every check in both profiles. 
The supported claim is therefore that the profiles are equivalent for file, header, record and allele questions and divergent by design for per-sample structure queries, the condensed profile storing sample values as vectors rather than as per-sample resources.
...
VCF-RDFizer supplies a separate implementation: release 3.0.2 constructs file and record context with RML mappings and produces expanded or condensed sample structures through streaming emitters~\cite{vcfrConverter2026}. 
All results in Sections~\ref{sec:evidence}--\ref{sec:integration} were produced with VCF Core 2.1.1 and VCF-RDFizer 3.0.2 together with the corrections that the cross-producer check prompted, which are not yet in a tagged converter release; the exact commits are recorded with the reproduction scripts. 
Converting the Section~\ref{sec:profiles} example emits 225 expanded and 159 condensed triples, drawing on 79 and 69 distinct VCF Core terms respectively. 
Every emitted class and property is declared in VCF Core 2.1.1, and both graphs satisfy the shared and VCF 4.5 SHACL profiles when the shapes are loaded with the vocabulary and RDFS inference, as the vocabulary's own validation does. 
...
The container image is published only under a mutable \texttt{latest} tag, so reproduction must cite an image digest rather than a version; and the toolchain inside that image is not independently pinned. 
The figures quoted for the Section~\ref{sec:profiles} illustration --- 172 expanded and 149 condensed triples --- describe the hand-authored illustration file, not converter output; the converter emits more because it also materializes allele resources and per-sample genotype and allele-call resources that the illustration omits for readability.

The recorded validation run accepts 21 fixtures and includes a negative control and 35 targeted regression tests. 
...
The input is a pinned four-site, single-sample VCF 4.5 fixture; the annotations are a local, explicitly synthetic snapshot of five flagged alterations, labelled as such in the file, and local rather than federated so the example returns the same answer on every run. 
...
A third question --- \emph{which alterations detected in a tumour have reviewed therapeutic evidence for the patient's cancer type, and what do the matched-normal observations show?} --- would join curated interpretation evidence such as CIViC~\cite{griffith2017civic} to normalized alterations, then retrieve tumour and normal calls through their specimen links. 
As above, a missing normal call remains missing evidence rather than proof of tumour specificity.
...
All 104 constructs in the curated VCF 4.5 inventory are represented, and the claims that matter have been executed rather than asserted: the specification-derived checks hold across an independent converter for the subset tested, all 35 records round-trip through structured properties, and one integration example returns answers authored in advance together with their source provenance.
...
The clinical scenarios in Section~\ref{sec:clinicalqueries} remain future work; the representation they would require is in place and checked.
