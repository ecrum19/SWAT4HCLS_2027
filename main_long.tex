...
The two programs share an author but no code: the assessment materializes graphs from a Python fixture builder, the converter from RML mappings and streaming emitters. 
Producer independence here therefore means independent implementations, not independent authorship, and the check is correspondingly weaker than a third-party reproduction would be.
...
Of the 39 requirements, 34 agree on every check, and for 28 of those every check passes for both producers. 
Where the two agree by both failing, the cause is uniform: all such checks are condensed-profile structure queries, the documented storage trade-off, on which the producers agreed before and after the corrections below.
...
Across the 39 exercised requirements the profiles return the same outcome on 252 of 286 comparable checks. 
The 34 disagreements are not scattered: they fall on nine requirements, every one of which reads a FORMAT-scoped value --- genotype structure, trailing-field missingness, sample-local allele and base-modification items, and repeat components. 
The record-level file, header, declaration and allele questions agree on every check in both profiles. 
The supported claim is therefore that the profiles are equivalent for file, header, record and record-level allele questions, and divergent by design wherever a query walks per-sample resources, the condensed profile storing sample values as vectors. 
Two of the nine are local-allele requirements, so the paper's headline feature is among those a condensed graph answers less completely.

\paragraph{Answers to the three questions.}
For RQ1, 53 of the 94 requirements have at least one executed case and the overwhelming majority pass; meaning preservation is demonstrated where tested and untested elsewhere. 
For RQ2, 17 of the 36 version-conditioned requirements are tested, and the two sample profiles behave as documented --- equivalent above the sample level, divergent below it. 
For RQ3, 27 of the 38 requirements on the provenance chain an integration query must traverse are tested, and Section~\ref{sec:integration} exercises that chain end to end. 
None of these fractions is a conformance claim; each is the share of requirements for which behaviour has been checked at all.
...
The Section~\ref{sec:evidence} assessment was produced with VCF Core 2.1.1 and the vocabulary's own materializer, with no converter involvement. 
All converter-dependent results (Sections~\ref{sec:crossproducer}--\ref{sec:integration}) additionally used VCF-RDFizer 3.0.2 together with the corrections that the cross-producer check prompted, which are not yet in a tagged converter release; the exact commits are recorded with the reproduction scripts. 
One further caveat applies on the vocabulary side: the QUAL datatype guidance discussed in Section~\ref{sec:crossproducer} was added after the 2.1.1 tag and is not part of the cited release, which the results otherwise use unmodified. 
Converting the Section~\ref{sec:profiles} example emits 227 expanded triples, two more than before the typed-accessor correction. 
Every emitted class and property is declared in VCF Core 2.1.1, and both profiles' graphs satisfy the shared and VCF 4.5 SHACL profiles when the shapes are loaded with the vocabulary and RDFS inference, as the vocabulary's own validation does. 
...
After those interpretation and mapping steps, the joined query would return the patient, assessment, applicable guideline, source file and supporting call depth, reaching the depth through the same declaration-to-value path that Section~\ref{sec:integration} executes. 
The clinical assertions would come from an application schema that is not part of VCF Core, and a depth filter illustrates access to retained sample evidence without defining a prescribing threshold.

\paragraph{Rare-disease investigation.}
A second question --- \emph{which candidate alterations occur in genes associated with diseases that share the patient's recorded phenotypic features, and which retain sufficient source evidence for review?} --- would join specimen-linked calls to normalized alterations, gene associations and phenotype descriptions such as HPO~\cite{hpo}. 
The critical constraint is that absence of a row in a variant-only VCF is not evidence that a parent is homozygous reference: the vocabulary retains observations, while completeness and segregation analysis require evidence it does not carry.
