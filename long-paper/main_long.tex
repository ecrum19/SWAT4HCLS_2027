...
A specification-derived assessment of 94 requirements over VCF 4.1--4.5 supplies queries and expected answers authored in advance. 
Replaying all of it over graphs from a second, separately implemented producer --- every fixture, every case that has a test --- gives 840 checks, 693 passing for both producers. 
The 102 checks where they differ span 15 requirements, and examining each shows that none arises from the vocabulary being unable to express something: every absent property is one the release declares and the converter does not yet write. 
A semantic round-trip recovers the fixed columns of all 109 records and all 151 sample genotypes, in the expanded profile, from structured properties alone, and one integration example answers an allele-dependent question under VCF 4.5 local alleles with checked answers and recovered provenance. 
