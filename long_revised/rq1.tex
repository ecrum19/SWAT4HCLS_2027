...
\subsection{Method}
\label{sec:m-rq1}
We assess correspondence between VCF specification and VCF Core in both directions: from specification requirements to their \textit{vcfcv} representation, and from the vocabulary to a curated inventory of source constructs.
...
From the text, \emph{requirements} were extracted programmatically, see \href{https://github.com/ecrum19/SWAT4HCLS_2027/blob/main/RQ1/EXPERIMENT.md}{\path{RQ1/EXPERIMENT.md}} for details.
A \emph{requirement} expresses a specification rule extracted as a question with a checkable answer, following established competency-question and requirements-specification practices~\cite{gruninger1995methodology,suarezfigueroa2009orsd,lot2022}.
...
R27 concerns the local-allele mapping shown on line 9, where \texttt{ALT=A,C} and each sample names a different single local allele: \texttt{LAA=2} for \texttt{S1} and \texttt{LAA=1} for \texttt{S2}.
Since \texttt{LAD} is declared \texttt{Number=LR}, its first value is the \texttt{REF} depth and its second belongs to that sample's local \texttt{ALT} allele.
The depth 30 therefore belongs to global ALT allele 2, "C", for \texttt{S1}, whereas the depth 9 at the same local position belongs to global ALT allele 1, "A", for \texttt{S2}.
Listing~\ref{lst:rq1-example} retrieves that mapping; restricted to line 9, it expects the rows \mbox{\texttt{(2, S1, 0, 0, G, 20)}}, \mbox{\texttt{(2, S1, 1, 2, C, 30)}}, \mbox{\texttt{(2, S2, 0, 0, G, 12)}}, and \mbox{\texttt{(2, S2, 1, 1, A, 9)}} for record position, sample, local index, global index, allele, and depth.
...
R21 concerns missingness, which line 11 of Listing~\ref{lst:vcf-example} exhibits.
...
The two remain distinguishable, so the expected results for line 11 are \texttt{(S1, ".")} and \texttt{(S2, unbound)}.
...
\textbf{87} of these constructs have a validation rule (i.e. a SHACL shape or a decoded check) testing a stated requirement.
% TODO: Fix this section and add a reference to the readme in the repo for more details
